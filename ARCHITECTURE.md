...
| `CandidateIdea` (+ `CrossDomainTransferPayload`, `ZeitgeistSynthesisPayload`) | §3 | id, runId, generationId, agenomeId, subtype, title, summary, claims[], evidenceRefs[], status; payloads per DATA_MODEL.md | **yes (runtime·verifier·selection·projection)** |
