/**
 * HealthIndicator — the operator's cockpit gauge and the continue-vs-switch-to-replay signal during
 * the 10-minute window (GET /runs/:id/health). Surfaces the one runtime read Langfuse can't give:
 * current generation, candidates in flight, last-event age, caps consumed. Stalled = the cue to drop
 * a rung on the fallback ladder. TS-strict port of
 * docs/doppl-design-system/components/observatory/HealthIndicator.jsx (adherence-clean — var() tokens).
 */
...
interface HealthStateSpec {
  color: string;
  glyph: string;
  label: string;
}

const STATE: Record<HealthStatus, HealthStateSpec> = {
...
export function HealthIndicator({
  ...
  const pulsing = status === 'stalled' || (mode === 'live' && status === 'healthy');
  return (
