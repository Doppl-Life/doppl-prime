...
describe('ThemeToggle + useTheme — dark/hc/light cycle, persisted to localStorage', () => {
  // spec(§12): the toggle cycles dark→hc→light, applies the matching class to document.documentElement
  ...
  // spec(§12): a persisted theme is restored on boot; an unset/invalid value → the dark default.
  it('test_theme_restored_on_boot', () => {
    localStorage.setItem('doppl-theme', 'hc');
    render(<ThemeToggle />);
    expect(document.documentElement.classList.contains('hc')).toBe(true);
    reset();
    ...
    render(<ThemeToggle />);
    expect(document.documentElement.classList.contains('hc')).toBe(false);
    expect(document.documentElement.classList.contains('light')).toBe(false); // dark default
  });
});
