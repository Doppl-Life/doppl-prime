...
|---|---|---|
| `RunEventEnvelope` | §4 | Strict 14-field envelope; `actor` = closed 7-role union (operator/runtime/agenome/critic/check_runner/selection_controller/system); `sequence` sole ordering key; `occurredAt` display-only; generic object `payload` (P0.10 narrows per-type); `schemaVersion` + exported `CURRENT_SCHEMA_VERSION=1`. Frozen in `packages/contracts` (P0.1); enforced by field-name schema-snapshot (`spec(§4)`). |
| `RunEventType` | §4 | Closed 25-member registry incl. all 7 failure/terminal types (RISK-006); rejects unlisted values. Frozen in `packages/contracts` (P0.1); member-set snapshot (`spec(§4)`). |
