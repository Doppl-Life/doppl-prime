...
## <a id="27"></a>27. The app-layer injection-isolation chokepoint (candidate-as-DATA) — instruction candidate-independent, candidate sentinel-wrapped in a separate message, ONE generic chokepoint
...
## <a id="28"></a>28. Check-runners = a closed descriptor-registry (the allowlist gate) + a PARALLEL closed pure-impl-map; the harness resolve→started→run-or-skip→completed, every invocation emits one validated CheckResult, every skip a FIXED reason
...
## <a id="29"></a>29. The immutable-anchor runtime LOAD path enforces the properties the contract can't pin — full-member-set COMPLETENESS + a flag re-assert read through an `unknown`-typed local — and loads from a deep-frozen const; source PROVENANCE is the boot caller's obligation
...
## <a id="30"></a>30. A gateway-routed evidence producer OWNS its output's identity/correlation fields — the model fills ONLY evidence via a permissive strip-parse, then the assembled whole is validated against the strict frozen contract (the model never controls its own identity — reward-hacking defense)
...
Persistence + scope discipline: an accepted review → `critic.reviewed` (payload = the validated review); a rejected/un-repairable output → `output_schema_rejected` + NULL for that mandate (NEVER a fabricated review — a rejection is a failure event, not a silent pass). The producer returns the evidence array ONLY (no winner/selection/mutation surface — rule #6 emit-only, pinned by an "exactly the N evidence keys" test). Every request goes through the P4.4 isolation chokepoint (lesson §27); every persist through `store.append` (forbidden #4); no energy debit (rule #8 — that's P3's ledger).
...
## <a id="31"></a>31. The DECISION aggregate is computed by trusted code, never the model — the model supplies the per-axis inputs, the runner does the deterministic weighted math over the immutable rubric; the held-out judge sits outside the loop and PERSISTS its result as the authoritative `judge.reviewed` record
...
The held-out judge produces the acceptance metric that decides "gen N+1 beats gen N" — the single number an evolved agenome would most want to move (rule #6 anti-reward-hacking). So the MODEL never produces that number. The split (a step beyond lesson §30, which kept the model from setting its output's IDENTITY — this keeps it from setting the DECISION AGGREGATE):
...
## <a id="33"></a>33. A retrieval-grounded check stays a PURE adapter via the CALLER/adapter split — the caller fetches retrieval + persists + re-threads; the adapter scores candidate-vs-results as DATA — keeping rule #3 non-executing and rule #7 replay trivial; absent results → skipped, never a false grounding
...
## <a id="34"></a>34. A live-rerun-with-replay-fallback = try-live-through-the-SAME-gated-path / else-serve-the-recorded-AUTHORITATIVE-result (read-only, no re-sample, no fabrication) — and filter the fallback to a USABLE verdict so it can't serve the triggering skip
...
## <a id="35"></a>35. A type-blind whole-value redaction CORRUPTS a non-string field whose key merely CONTAINS a sensitive substring — redact a sensitive-key value UNLESS it is number/boolean (never a credential); the structured-value-redaction tests are the no-weakening guardrail
