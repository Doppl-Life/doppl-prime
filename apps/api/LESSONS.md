...
## <a id="34"></a>34. The run-health signal is a read-only log-derived projection; operations-in-flight = COUNT-based unpaired markers; caps-consumed is CLAMPED to the ceiling
...
## <a id="35"></a>35. The SSE run-event stream is delivery-only + non-authoritative — a demo-owned bridge POLLS `readByRun` past the cursor (no kernel-file edit), SSE `id`=sequence so `Last-Event-ID` resume is gap/dup-free, injectable timing keeps tests timer-free
...
## <a id="36"></a>36. Runtime self-observability is built ahead of the live worker + injected-everything — the kernel-logger stamps the §4 correlation IDs to an injected sink (console local, NOT scrubbed; external REUSES the §28 boundary), the heartbeat is an injected-clock throttle + pure staleness predicate, both structurally barred from the append path
...
- **Structured kernel-logger:** stamps the §4 envelope correlation IDs (`runId` required + `generationId?`/`agenomeId?`/`correlationId?`) onto every record, emitted to an **injected sink (default console)**. The **local `log()`/console path is NOT scrubbed** — console is inside the **process trust boundary** (LESSONS §32 precedent: `request.log.error` is OUTSIDE the rule-#4 event-log/Langfuse/UI boundary; secrets never reach the logger's input by the env-only structural guarantee anyway). The **only EXTERNAL path (`emitExternal`) REUSES the §28 `createEmitBoundary`** (ceiling-then-scrub before any external sink, rule #4 — never reimplemented; **no boundary injected = no-op**, never a raw external emit).
...
**Rule:** Build runtime-observability primitives ahead of the live worker + injected-everything: a structured logger stamps the §4 correlation IDs to an injected sink (console local + UNSCRUBBED per the §32 trust boundary; external REUSES the §28 scrub, never reimplemented, no-boundary=no-op); a heartbeat is an injected-clock throttle (≤1/intervalMs, no real timer) + a pure staleness predicate; both are structurally barred from the append path (import-ban test) and emit no `run_events` (a side signal, not in the closed registry); no external metrics stack (console + injected only); the worker-loop + `/health` wiring defer to P3/integration.

## <a id="37"></a>37. Run `format:check` in the PER-SLICE gate, not just at /preflight — else prettier drift accumulates across committed slices to the phase-exit boundary
