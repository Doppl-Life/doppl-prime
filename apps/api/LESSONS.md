...
## <a id="31"></a>31. The DECISION aggregate is computed by trusted code, never the model — the model supplies the per-axis inputs, the runner does the deterministic weighted math over the immutable rubric; the held-out judge sits outside the loop and RETURNS its result (no dedicated completion event)

**Date:** 2026-06-21.
**Source slice:** verifier track P4.8 (`verifier-005`; commit hash recorded at round close); `apps/api/src/verifier/judge/judge-call.ts`.
...
Cross-track seam (no dedicated completion event): there is NO `judge.reviewed` in the 36-member registry — the judge's paired completion is `fitness.scored`, which is SELECTION's (P5). So the runner emits only the `judge.review_started` marker (actor `selection_controller` — the held-out judge has no `judge` actor in the closed 7-role union, so it acts as the selection-deciding authority; a role CHOICE, recorded as an arch note, not a contract change) and RETURNS the `JudgeAcceptance` (`{axisScores, acceptanceMetric, policyVersion}`). Selection folds it into the OPEN `FitnessScore.components` record (the designed cross-track seam — no frozen-contract gap) and persists `fitness.scored` (the rule-#7 replay home). `JudgeModelOutput`/`JudgeAcceptance` are app-level types (the model-facing + return shapes), not frozen contracts (lesson §30 precedent).

**Rule:** A decision/fitness aggregate an agent would want to move is computed by TRUSTED CODE, never the model — the model supplies only the per-axis inputs (strip-parsed so a model-sent aggregate is dropped), the runner does the deterministic weighted math over the immutable rubric (replay-recomputable; iterate the closed axis set so non-axis weights are excluded); load the rubric only from the immutable const (lesson §29), pass the candidate as DATA (lesson §27), keep the judge outside the loop with no selection surface, and RETURN the result for the owning track to persist when no dedicated completion event exists (the open component record is the cross-track seam).
