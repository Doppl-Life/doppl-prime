...
## <a id="37"></a>37. Seeded-RNG replay-determinism by construction — persist the RNG OUTCOME not the derived value, and keep the replay source PRNG-free (extends §30/§31)
...
## <a id="38"></a>38. RunCaps enforcement + kill switch are PURE decisions (loop emits) — caps fail-closed with NO trait input (rule #1 by shape); the kill switch maps each non-terminal to its §3-LEGAL terminal, exclude-and-drain (extends §33)
...
## <a id="39"></a>39. Success-only energy ledger — a PURE estimate+reconcile debit; llm `actual` from real ProviderMeta only (PM required by shape, no estimate leakage); cumulative folds ACTUAL; failures yield NO EnergyEvent (rule #8 by shape); emission deferred (extends §33)
...
## <a id="40"></a>40. Run the FULL `/preflight` gate (incl `format:check`) EVERY slice — a subset check lets prettier drift land + forces a retro `chore(format)`
...
## <a id="41"></a>41. A closed recording-event registry must cover EVERY reachable state-machine terminal — audit ALL terminals at once, not one gap at a time
...
## <a id="42"></a>42. The generation loop is pure ORCHESTRATION — it appends only kernel-owned events and consumes injected-seam events as DATA, never authoring a seam-owned type
