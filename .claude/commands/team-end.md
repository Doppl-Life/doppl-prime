...
**Worktree:** <`../Capstone-<track>` (branch `track/<track>`), or "root checkout (single-track)">
...
2. **Worktree teardown.** Once the branch is merged (or the team is fully done with the worktree), remove it:
   ```bash
   git worktree remove ../Capstone-<track>     # add --force ONLY after confirming no uncommitted work
   ```
