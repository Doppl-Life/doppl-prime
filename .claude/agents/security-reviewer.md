...
3. Read the area's cross-doc invariants table in `apps/api/CLAUDE.md` — the pin matrix.
...
<!-- ▼ EXAMPLE BLOCK [id=safety-invariant-cross-checks]: project safety-invariant cross-checks — replace wholesale with the project's actual key safety rules + the specific cross-checks for each. ▼ -->
   ...
   - **Caps enforced in the kernel, not prompts** — grep the slice for cap/budget/limit terms (population, generations, energy, depth, tool-calls, wall-clock); confirm every cap is checked in `apps/api/src/runtime/` runtime code, NOT asserted via agenome prompt text. Confirm any `spawnBudget` is treated as a hint clamped to `min(remaining caps)`, never authoritative. FINDING if a cap is enforced in prompt text or a budget hint is trusted unclamped.
   - **Append-only authoritative event log** — confirm any write to `run_events` goes through the single append-only writer in `apps/api/src/event-store/`; no UPDATE/DELETE/in-place edit of historical rows; no projection (in `apps/api/src/projections/`) treated as authoritative. Trace writers with `codegraph_callers`. FINDING on any bypass of the append-only writer or any historical mutation.
   - **No arbitrary code execution (allowlisted check-runners)** — confirm check-runners in `apps/api/src/check-runners/` are non-executing adapters off an allowlist; no `eval`/`exec`/`spawn`/`vm`/dynamic `require` on candidate-derived input; unregistered or execution-requiring checks must return `check.completed status=skipped+reason`. FINDING on any execution surface or any unregistered check that runs rather than skips.
   - **Secret redaction at the persistence boundary** — confirm provider keys/secrets never enter prompts, event payloads, Langfuse traces, or UI payloads; confirm the redaction scrub runs at the persistence boundary BEFORE append AND before Langfuse emit (`apps/api/src/event-store/` + `packages/observability/`). FINDING if a new payload/log/trace path bypasses the scrub or can carry a secret.
   - **Candidate text is data, never instruction** — confirm model output is schema-validated before reaching critics/judges, and candidate text is passed only as DATA in a sentinel-delimited field, never interpolated into an instruction string. Grep for template-string interpolation of candidate/model text into prompts. FINDING on any prompt-injection surface (candidate text concatenated into an instruction).
   - **Immutable held-out judge / rubric / scoring policy** — confirm the held-out judge, its fixed rubric, and the scoring policy in `apps/api/src/verifier/` are not mutable by agents (no agent-reachable write path, no genome-supplied rubric/policy override). FINDING if an agent or genome can move the fitness anchor.
   - **Replay calls no providers** — confirm the replay path reconstructs from the persisted per-run RNG seed + outcomes and NEVER calls model/embedding/web providers. Trace replay (`apps/api/src/event-store/` reader) with `codegraph_callees` for any `ModelGateway`/embedding/web-search reach. FINDING on any provider call on the replay path.
   - **Energy debited only on successful productive calls** — confirm energy is debited only on a successful productive call; failed/retried/repaired attempts emit `provider_call_failed` and do NOT debit energy; confirm bounded retries + per-call timeout + wall-clock cap bound finiteness. FINDING if a failed/retried call debits energy or if a retry/timeout/wall-clock bound is missing.
   - **No SQLite; SDKs only behind ModelGateway** — confirm Postgres only (no SQLite import/driver/connection string), and provider SDKs appear only behind `apps/api/src/model-gateway/`, never in domain/runtime modules. Grep for SQLite and for provider-SDK imports outside `model-gateway/`. FINDING on any SQLite usage or any provider SDK imported in a domain/runtime module.

   Report PASS or FINDING per invariant with file:line + cited spec anchor.
