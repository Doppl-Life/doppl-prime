...
|---|---|---|---|
| `code-quality-reviewer` | At `/tdd` Step 8, **per the reviewer policy** in root `CLAUDE.md` (this project: `phase-boundary` — once at the phase-exit gate, dispatched by `/phase-exit`). | Implementer-side at per-slice; orchestrator-side at phase-boundary. Findings feed Step-9 categorization. | **Active** |
| `security-reviewer` | At `/tdd` Step 8, **per the reviewer policy** (this project: `invariant` — invariant-/security-touching slices only; `opus`). Mandatory on invariant-touching slices (caps, redaction, allowlist, injection, held-out judge). | Implementer-side. Critical findings escalate as Step-9 `Finding` → orchestrator → lead → human. | **Active** |
...
| `brief-drafter` | Definition only — not integrated into standard workflow without a quality trial first (see Brief-drafter quality trial below). | Orchestrator-side (manual invocation for trial). | **Deferred — quality trial required** |
...
**Reviewer policy (the toggle).** The Step-8 reviewer fan-out is gated by the **reviewer policy** in root `CLAUDE.md` "Reviewer subagents — Step-8 policy" — one of `off` · `invariant` · `every-slice` · `phase-boundary` per reviewer (this project: security `invariant`, code-quality `phase-boundary`). Per-slice reviews cover the slice **diff**, not whole files; at `phase-boundary` the surface is the **phase's accumulated branch diff + crossed trust boundaries** (root `CLAUDE.md` states the same rule). Tune it to trade review depth for per-slice tokens.
