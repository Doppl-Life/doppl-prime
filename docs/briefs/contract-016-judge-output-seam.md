...
- **Orchestrator doc rows (single operator authored):** `ARCHITECTURE.md` §7/§8 + Appendix-A (`JudgeResult` row + RunEventType/payload-map row updates); `apps/api/CLAUDE.md` cross-doc table (new `JudgeResult` row + RunEventEnvelope/RunEventType/payload-map row fixes + lessons index §32 (renumbered from §20 on merge)); `IMPLEMENTATION_PLAN.md` P0.16 + Log. **Safety-relevant:** any closure break, any way an agent moves a judging axis / scoring authority through the output, any secret-smuggle, or a missing required field that lets replay re-judge is a Finding.
