...
This project runs the **agent-team orchestrator + implementer pattern** (three roles + human). Same slash commands, same Step-9 routing matrix, same N+2 commit cadence, same cross-doc invariants discipline as the universal pattern. **Adaptations are project-shaped:** this project's stack, code areas, phase plan (`P0 / P1 / P2 / P3 / P4 / P5 / P6 / P7 / PD`), forbidden patterns, and architecture are its own.
...
This project has **two code areas** and a frozen-contracts package that forks the build into **5 parallel tracks** (`contract → kernel → verifier → selection → demo`):

```
Capstone/
├── packages/
│   ├── contracts/                      # Frozen Zod schemas (Appendix-A models) — contract track, Phase 0; forked-from before tracks split
│   │   └── LESSONS.md                  # (if contract-track lessons accrue)
│   └── observability/                  # Langfuse adapter + redaction (thin)
├── apps/
│   ├── api/                            # ← CODE AREA 1: the backend (Doppl kernel + subsystems)
│   │   ├── CLAUDE.md                   # Backend conventions + forbidden patterns + cross-doc invariants table
│   │   ├── LESSONS.md                  # Backend lessons (§1+)
│   │   └── src/{runtime,event-store,model-gateway,verifier,check-runners,selection,projections,routes}/
│   └── web/                            # ← CODE AREA 2: the React dashboard
│       ├── CLAUDE.md                   # Frontend conventions + forbidden patterns
│       ├── LESSONS.md                  # Frontend lessons (§1+)
│       └── src/{lib,components/lineage,components/run,components/evidence,routes}/
└── docs/
    ├── planning/                       # Architecture-planning artifacts
    ├── gap-audits/                     # Gap-audit fan-out outputs
    ├── briefs/                         # Numbered /tdd briefs (<track>-NNN-<task-id>-<topic>.md in multi-track)
    ├── sessions/                       # Session docs (<track>-NNN-… in multi-track)
    └── team-handoffs/                  # /team-end outputs (<track>-NNN-… in multi-track)
```

- **Track prefixes** (the only valid `<track>` values, from the `IMPLEMENTATION_PLAN.md` Track map): `contract`, `kernel`, `verifier`, `selection`, `demo`. Each track works in its own worktree/branch; numbered docs are track-prefixed and `NNN` counts independently per prefix.
- **No SQLite anywhere** — Postgres-only event store; tests run against the real Postgres `run_events` log.
...
Command descriptions are injected by the harness per command; root `CLAUDE.md` "Slash commands" keeps only the role pairing. Pairs: lead `/team-start`+`/team-end`; orchestrator `/orchestrate-start`+`/orchestrate-end`; implementer `/session-start`+`/session-end`; plus `/tdd`, `/wired`, `/context-check`, `/preflight`, `/run-tests`, `/check-arch` (+ optional `/eval`). _(Single-operator: no `/team-start`/`/team-end`.)_
...
- **Event-sourced posture:** the Postgres `run_events` log is the single append-only source of truth; projections/SSE/Langfuse/Neo4j are all derived and rebuildable. Write the event, rebuild the projection — never treat a projection as authoritative. Replay reconstructs from the persisted per-run RNG seed + outcomes and **never** calls model/embedding/web providers.
- **Forbidden patterns — backend (`apps/api/`):** (a) calling a provider SDK from domain/runtime — go through `ModelGateway`; (b) enforcing a cap/permission in prompt text — caps are kernel invariants; (c) writing to `run_events` outside the append-only writer — bypasses sequence/redaction/validation; (d) treating a projection as authoritative — write the event, rebuild the projection; (e) re-calling a model/embedding/web provider on the replay path. See `apps/api/CLAUDE.md`.
- **Forbidden patterns — frontend (`apps/web/`):** (a) mutating authoritative runtime state from the dashboard — UI is read-only, commands go via REST; (b) treating SSE as source of truth — resync from last sequence; (c) status by color alone — shape+label+icon (projector/colorblind-safe); (d) shipping provider keys to the client. See `apps/web/CLAUDE.md`.
- **Layer dependency rule — backend:** contracts → infra ports → domain/runtime → projections → routes; domain imports only contracts + ports. Provider SDKs live only behind the `ModelGateway`.
- **Layer dependency rule — frontend:** reads projections via the API client (`apps/web/src/lib`); never imports backend internals.
- **Key safety rules:** see root `CLAUDE.md` "Key safety rules" (9 invariants — caps in kernel not prompts; append-only authoritative log; no arbitrary code execution / allowlisted check-runners; secrets never in prompts/events/traces/UI; model output untrusted until schema-validated; immutable held-out judge; provider-free replay; success-only energy spend; Postgres-only / SDKs behind `ModelGateway`).
- **Cross-doc invariants:** the `apps/api/CLAUDE.md` table tracks typed models (Appendix-A) mirroring `ARCHITECTURE.md` sections; schema-snapshot tests run on every Appendix-A model. Field changes require atomic doc edits across `packages/contracts/` (frozen Zod), `apps/api/CLAUDE.md`, and `ARCHITECTURE.md`.
...
| Conventions / patterns | `apps/api/LESSONS.md` (prose) + `apps/api/CLAUDE.md` (index) | On-demand |
...
- **New lesson** → next anchor in `apps/api/LESSONS.md` + row in the `apps/api/CLAUDE.md` index. Hot-routed at Step 9.
- **New convention** → entry in `apps/api/CLAUDE.md` Forbidden patterns or root `CLAUDE.md` Key safety rules + a `LESSONS.md` entry if durable.
- **New cross-doc invariant** → row in the `apps/api/CLAUDE.md` table + atomic `ARCHITECTURE.md` edit.
...
**Don't** add project *state* to scaffolding files — state lives in `IMPLEMENTATION_PLAN.md`. **Don't** rename the cross-referenced files (`IMPLEMENTATION_PLAN.md`, the `CLAUDE.md` files, `apps/api/LESSONS.md`, `docs/team-protocol.md`, `docs/orchestrator-briefing.md`, `docs/tdd-brief-template.md`) casually — they're named inside slash command bodies; renaming is a multi-file ripple.
