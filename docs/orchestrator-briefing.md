# Orchestrator Session Briefing — Doppl
...
You're picking up the **orchestrator role** — one teammate on a Claude agent team. Your job is to drive Doppl forward. The active phase plan, deadlines, and currently-in-progress state live in `IMPLEMENTATION_PLAN.md` — this briefing stays **state-free** so it doesn't drift.

> **Architecture sentence to preserve as the project's posture:** *It's not the agent — it's the kernel that breeds the agents; the event log is the truth, and the held-out judge is the floor the organism cannot lift.*
>
...
Capstone team lead / kernel-builder (GitHub **SiWarlock**) building Doppl for the GauntletAI capstone (showcase **Jun 29 2026**). Strong systems/ML engineer who drives the planning chain decisively and fast. They prefer:

- **Concise, decision-oriented** communication — surface trade-offs plus a recommendation; don't over-ask.
- **Exhaustive accountability** — every finding/decision dispositioned, not just headlines.
- **Pragmatic MVP/prototype posture** with the load-bearing safety rails kept (caps, redaction, allowlist, held-out judge).
- **Commits direct to main** on this repo — no PR ceremony for routine slices.
...
**Project:** Doppl. An evolutionary multi-agent system that breeds verifiable ideas under selection pressure

Doppl is an experimental **agental-evolution runtime**. A bounded population of agent **genomes** generates candidate ideas — two subtypes (`cross_domain_transfer`, `zeitgeist_synthesis`) sharing **one lifecycle** — which an adversarial **critic council** + a **held-out judge** + **allowlisted objective checks** score; weak lineages are culled, strong ones **fuse** (two-parent crossover + output synthesis) and **mutate** into later generations that measurably beat earlier ones on a held-out rubric. It's a custom TypeScript kernel: a **Postgres append-only `run_events` log is the source of truth** (projections / SSE / Langfuse / Neo4j are all derived and rebuildable); a provider-agnostic **ModelGateway** (OpenRouter primary, direct-OpenAI embeddings, live web-search grounding) is the only path to providers; **Langfuse** gives observability; a **React Flow** lineage dashboard renders live + replay. Built across **5 parallel tracks** — contract → kernel → verifier → selection → demo — with Zod contracts frozen in `packages/contracts` before the tracks fork. Local-first demo with a replay fallback (replay reconstructs from the persisted per-run RNG seed + outcomes and never calls providers).
...
**Repo:** `Capstone/`. Pushes go to **origin** only.
...
1. **Plan + scope** — maintain `IMPLEMENTATION_PLAN.md`; decide where new work fits in the P0 / P1 / P2 / P3 / P4 / P5 / P6 / P7 / PD phase plan.
...
| `/orchestrate-end` Step 7 | Orchestrator | `IMPLEMENTATION_PLAN.md` + `apps/api/LESSONS.md` + `apps/api/CLAUDE.md` index + `ARCHITECTURE.md` prose + `docs/briefs/NNN-*.md` + optional orchestrator session doc. **Round terminal commit.** | **Only if a remote exists — to origin** |
...
5. **Event-sourced — the log is the only truth.** The Postgres `run_events` log is append-only and authoritative; never edit history in place. Every projection (SSE / Langfuse / Neo4j / lineage) is **derived and rebuildable** — write the event, then rebuild the projection; never treat a projection as authoritative.
6. **Caps live in the kernel, never in prompts.** Population / generations / energy / depth / tool-calls / wall-clock caps are enforced in the runtime kernel as invariants; `spawnBudget` is only a hint clamped to `min(remaining caps)`. A cap or permission expressed in agenome prompt text is a forbidden pattern.
7. **Providers only via the ModelGateway.** Provider SDKs live behind the `ModelGateway` only — never imported in domain/runtime modules. Energy is debited **only on successful productive calls**; failed/retried/repaired attempts emit `provider_call_failed` and do NOT debit energy. The **replay path never calls** model/embedding/web providers — it reconstructs from the persisted per-run RNG seed + outcomes.
8. **Untrusted model output + immutable judge.** Model output is untrusted until schema-validated (accept / repair ≤1 / reject); candidate text reaches critics/judges only as DATA in a sentinel-delimited field, never interpolated into an instruction string. The **held-out judge, its fixed rubric, and the scoring policy are immutable to agents** — the bedrock fitness anchor. Check-runners are **allowlisted, non-executing** adapters (no arbitrary code execution); unregistered/execution-requiring checks return `check.completed status=skipped+reason`. **Secrets/provider keys** never enter prompts, event payloads, Langfuse traces, or UI payloads — a redaction scrub runs at the persistence boundary (before append AND before Langfuse emit). **No SQLite — Postgres only.**
9. **Layer dependency rule.** Backend import direction: contracts → infra ports → domain/runtime → projections → routes; domain imports only contracts + ports. Frontend (`apps/web`) is **read-only over projections** — it never mutates authoritative runtime state (commands go via REST), never treats SSE as source of truth (resync from last sequence), never imports backend internals, and never ships provider keys to the client. Zod contracts are **frozen in `packages/contracts` before the tracks fork**; schema-snapshot tests cover every Appendix-A model.
