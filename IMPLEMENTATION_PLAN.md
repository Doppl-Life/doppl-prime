...
- **[FINDING — material; ESCALATED 2026-06-23 lead→user; pending decision] Live structured-output HTTP 400 blocks the LIVE-path winner (gateway strict-schema ⊄ OpenAI strict subset).** PD.8c live run RAN-LIVE (8/10) and PROVED rules #1/#4/#7/#8 hold on a real live run, BUT every `population_generator` call → OpenRouter **HTTP 400**: the OpenRouter adapter sends `response_format:{strict:true, schema:z.toJSONSchema(CandidateContent)}` and `CandidateContent` is a discriminated union (LESSON §91) → root `anyOf` + an optional field, which OpenAI's strict structured-output subset rejects (requires a root `object` + every key in `required`) → all generation fails → `run.failed{no_scored_survivor}`, NO live winner. Model+key verified working (plain curl → 200). Likely also hits critic/judge/fusion (all STRUCTURED_ONLY). The **creds-free replay demo-of-record (PD.8a `demo-recorded-001`) is UNAFFECTED** (replays a winner). Fix options: (1) a `toOpenAiStrictJsonSchema` transform (root object + nullable-and-required optionals + strip unsupported keywords); (2 — rec) relax `strict:true`→`json_object`/`strict:false` + lean on the EXISTING gateway validate/repair(≤1)/reject discipline (LESSON §23/§91 — provider strict-mode was an optimization, not the safety mechanism). **A NEW fix slice beyond PD.12; pending the user's approach + now-vs-follow-up call.** _(origin: 2026-06-23 PD.8c live exec; demo track; DELETE after the structured-output fix lands + a live re-run produces a winner)_
...
### PD.14 — Wire web↔API for the real demo UI (Vite proxy + env baseUrl + a REAL web↔API smoke) — FINDING, fix this round
