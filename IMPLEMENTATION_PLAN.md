...
- **Unified VerifySeam adapter wires `selectCriticMandates`→`runCouncil` + checks + judge behind the loop's verify port — IN PROGRESS as P4.12 (verifier-013).** `createVerifySeam(deps)` composes the rotation+council+checks+judge behind the kernel loop's frozen `verify` port (rngSeed via `readRngSeed(config.runConfig)`, generationIndex from the persisted `generation.started{index}` — Option A). The PRODUCTION injection (`seams.verify = createVerifySeam(...)` into `runGenerationLoop`) lands at **selection's boot composition root (P5)** after the lead's `track/verifier`→cody merge + closing handshake. _(origin: verifier P4.7; verifier half → P4.12/verifier-013; production wiring → selection P5; DELETE after selection P5 injects the seam)_
