...
import type { OpenRouterModelClientInput } from '../model/model-gateway.ts';
import type { FitnessLensId, FitnessScheduleMode } from '../engine/scoring.ts';
...
export async function dashboardIndexPage(): Promise<string> {
  try {
    return await readFile(path.join(process.cwd(), 'web/dist/index.html'), 'utf8');
  } catch {
...
export function dashboardAssetPath(urlPath: string): string | undefined {
  ...
  return path.join(process.cwd(), 'web/dist', normalized);
}
