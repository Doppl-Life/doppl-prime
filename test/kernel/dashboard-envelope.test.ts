...
import { toDashboardEnvelope } from '../../src/kernel/dashboard-envelope.ts';
import { createMemoryEventRecorder } from '../../src/kernel/event-store.ts';
...
test('reshapes lifecycle payloads into the dashboard per-type contracts', () => {
  ...
  // run.started -> { startedAt }, not the canonical trace's { runId, caseId }.
