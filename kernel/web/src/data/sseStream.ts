...
import { RunEventEnvelope, RunEventTypeValues } from "./contracts.js";
...
export function createSseStream(options: SseStreamOptions): SseStreamHandle {
  ...
  const dispatchRawMessage = (raw: string) => {
    ...
    if (!parsed.success) {
      options.onError?.({ kind: "parse", detail: parsed.error.issues });
      return;
    }
  ...
  const startLive = () => {
    ...
    const url = `${baseUrl}/kernel/dashboard/runs/${encodeURIComponent(options.runId)}/stream?lastEventId=${lastApplied}`;
