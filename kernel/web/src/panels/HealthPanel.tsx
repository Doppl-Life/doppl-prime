...
export function HealthPanel(): JSX.Element | null {
  ...
  const consumed = health.capsConsumed;
  ...
  return (
    <section aria-label="Run health" style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      ...
      <Metric
        label="last event"
        value={formatEventTime(health.lastEventAt)}
      ...
      <Metric
        label="energy"
        value={`${consumed.energy ?? "—"}${capsCfg.energyBudget ? ` / ${capsCfg.energyBudget}` : ""}`}
        tip="Doppl-energy consumed vs the run's budget. Energy bounds total compute."
      />
      <Metric
        label="generations"
        value={`${consumed.generations ?? "—"}${capsCfg.maxGenerations ? ` / ${capsCfg.maxGenerations}` : ""}`}
      ...
      <Metric
        label="tool calls"
        value={`${consumed.toolCalls ?? "—"}${capsCfg.maxToolCalls ? ` / ${capsCfg.maxToolCalls}` : ""}`}
