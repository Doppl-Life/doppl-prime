...
test('exports a calibrator-facing run index', async () => {
  ...
  assert.equal(index.assayControl.heldOutJudge.referenceBenchmark.referenceStatus, 'withheld_reference_available');
  assert.equal(index.assayControl.heldOutJudge.referenceBenchmark.visibility, 'sealed_evaluator_only');
  assert.equal(index.assayControl.heldOutJudge.referenceBenchmark.contentIncluded, false);
  assert.equal(index.assayControl.heldOutJudge.referenceBenchmark.baseline.candidateId, index.assayControl.baseline.candidateId);
  assert.equal(index.assayControl.heldOutJudge.referenceBenchmark.survivor.candidateId, index.assayControl.survivor.candidateId);
  assert.equal(typeof index.assayControl.heldOutJudge.referenceBenchmark.delta.score, 'number');
  assert.ok(['doppl_wins', 'baseline_wins', 'tie'].includes(index.assayControl.heldOutJudge.referenceBenchmark.verdict));
  assert.ok(index.assayControl.heldOutJudge.referenceBenchmark.survivor.factors.length > 0);
