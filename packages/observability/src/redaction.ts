...
/**
 * Observability secret-redaction scrub at the Langfuse-emit boundary (KEY SAFETY RULE #4, §14).
 *
 * The observability TWIN of the shipped event-store `scrubEventPayload` (P1.2): the SAME composition
 * applied at a second boundary. The emit boundary (`./emit`) runs this on every payload BEFORE any
 * Langfuse export, so an unredacted secret never leaves the process into a trace.
 *
 * It COMPOSES the frozen `@doppl/contracts` `scrubSecrets` (key-format + key-name + secret-key layers,
 * never reimplemented — LESSONS 5) and adds the boundary-local env-value layer (§14, human-ratified
 * Option A): any payload string containing a loaded `process.env` secret VALUE is redacted, catching
 * secrets that match no key-format pattern and sit under no sensitive key-name (a Langfuse/DB
 * credential, an over-persisted raw model output).
 *
 * Re-composed boundary-locally on purpose (§14: the frozen scrub is key-format + key-name only; the
 * env-value layer applies where env loads). Mirrors `apps/api`'s twin — extract a shared pure helper
 * only when a THIRD boundary appears (P6.10 kernel-logger; YAGNI until then).
 *
 ...
 * `RunEventEnvelope.payload` is an open `z.record(z.string(), z.unknown())` and most event types fall
 * to the same generic open-key schema, so producer-controlled strings reach KEY positions — and a
 * non-format secret used as a key is exactly the class the env-value layer is the sole defense for
 * (the frozen scrub only catches key-FORMAT secrets + sensitive key-NAMES in keys).
 *
 * Key redaction carries DE-COLLISION (`[REDACTED]`, `[REDACTED]#2`, …, resume-cursor so it stays O(1)
 * amortized) — mirroring the frozen `scrubSecrets` pattern (the contract is immutable) so two distinct
 * secret keys redacting alike don't collapse and silently drop a value (LESSONS 3).
 *
 ...
 */
export function scrubObservabilityPayload(
  payload: unknown,
